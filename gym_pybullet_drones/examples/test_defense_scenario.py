...
async def ws_server():
    async def handler(ws):
        last_sent_t = -1
        while True:
            if LATEST_FRAME is not None and LATEST_FRAME["t"] != last_sent_t:
                try:
                    await ws.send(json.dumps(LATEST_FRAME))
                    last_sent_t = LATEST_FRAME["t"]
...
# --- CONFIG ---
NUM_DEFENDERS = 10
NUM_ENEMIES = 5 # This is now just for the scenario config
...
def run():
    ...
    print("[INFO] Starting WebSocket telemetry server on ws://localhost:8765...")
    ...
    while True: # Episode Loop
        ...
        scenario_config = {
            ...
            "spawn_radius": 80.0, # Closer for demo
        ...
        while not episode_over:
            ...
            global LATEST_FRAME
            drones = []
            for idx, state in enumerate(friendly_states):
                rpy = p.getEulerFromQuaternion(state['quat'])
                drones.append({
                    "id": int(state['id']), "role": "defender",
                    "pos": state['pos'].tolist(),
                    "rpy": list(rpy),
                    "vel": state['vel'].tolist(),
                    "rpms": action[idx].tolist() if action is not None else [0,0,0,0]
                })
            for idx, e in enumerate(current_enemies):
                drones.append({
                    "id": 100 + int(e['id']), "role": "attacker",
                    "pos": e['pos'].tolist() if isinstance(e['pos'], np.ndarray) else list(e['pos']),
                    "rpy": [0,0,0],
                    "vel": e.get('vel', [0,0,0]).tolist() if isinstance(e.get('vel'), np.ndarray) else [0,0,0],
                    "rpms": [5000, 5000, 5000, 5000]
                })
            LATEST_FRAME = {"t": i / env.CTRL_FREQ, "drones": drones}
